...
# Compare the relationship between the number of events occured for each cat and other aspects
...
# Find average number of events a cat take in a day day seperated by sex
avg_day <- cats_joined %>% 
...
# Making a graph from long and latitude
library(sf)
...
ggplot(data = month_final, mapping = aes(x = month_num, y = avg, group = 1)) +
  ...
  geom_text(data = rbind(month_final[which.min(month_final$avg), ], month_final[which.max(month_final$avg),]), aes(x = month_num, y = avg, label = avg), nudge_y = 50)
...
# Total number of events vs deployed hours -- there is one cat with missing deploy_hrs data
...
ggplot(data = cats_joined, aes(x = deploy_hours, y = total_number)) +
  ...
  geom_text(data = rbind(cats_joined[which.min(cats_joined$total_number), ], cats_joined[which.max(cats_joined$total_number),]), aes(x = deploy_hours, y = total_number, label=animal_id), size = 2.5, nudge_y = -100) +
  ggtitle("Cats New Zealand: Total number of events vs deployed hours") +
  labs(y = "Total number of events")
...
# Avg of events per cat in a day separated by sex
ggplot(data = avg_day) +
  ...
  ggtitle("Average number of events per cat in a day seperated by sex") 

# Crete a graph based on long and lat 
ggplot() +
